...
class Matrix (object):
	...
	# get largest spaces of digits needed for the matrix
	def maxSpaces(self):
		defaultSpaces = 1
		for i in range(self.row):
				for j in range(self.col):
					if (len(str(self.matrix[i][j])) > defaultSpaces):
						maxSpaces = len(str(self.matrix[i][j]))
					else:
						maxSpaces = 1
		return maxSpaces
	...
	# return a 2-D representation of the matrix with the 
	# numbers in neat columns right justified.
	def __str__ (self):
		s = ''
		numSpaces = self.maxSpaces()

		for i in range (self.row):
			for j in range (self.col):
				st = str(self.matrix[i][j])
				s = s + st.rjust(numSpaces)
				s = s + ' '
			s = s + "\n"
		return s
